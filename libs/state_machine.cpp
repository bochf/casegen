...
Graph * StateMachine::generate(const size_t rows, const size_t cols, size_t * states[]) {
  ...
  // the possible onnections between 2 states is one bit difference on the feature sets
  // that means it is switchable by one stop to turn on/off one feature
  ...
  for (size_t i = 0; i < rows; ++i)
    for (size_t j = 0; j < rows; ++j) {
      ...
      if (attr_diff.size() == 1) {
        EDGE_TYPE edge_type = attr_diff[0] + states_attr[i]->get(attr_diff[0]) * cols;
        m_StateEvent.link(i, j, edge_type);
        cout << "add link " << i << ", " << j << endl;
      }
    }
...
Graph * StateMachine::generate(const string & state_file) {
  ...
  if (!fs.good()) {
    cerr << "open file " << state_file << " error" << endl;
    return NULL;
  }
...
Graph * StateMachine::load(const string & matrix_file) {
...
void StateMachine::configure(const Properties & config) {
	...
	Properties::const_iterator it = config.find("ALGORITHM");
	if (it != config.end())
		algorithm = static_cast<IGraphTraveller::GT_ALGORITHM>(it->second);

	if (m_pTrasition && m_pTrasition->algorithm() != algorithm) {
		delete m_pTrasition;
		m_pTrasition = NULL;
	}
	if (NULL == m_pTrasition)
