...
class GraphTravellerBfs : public IGraphTraveller {
 ...
 public:
  ...
  static LinkList * shortestPath(const Graph * g, const Vertex * v1, const Vertex * v2);
...
class GraphTravellerDfs : public IGraphTraveller {
 ...
 protected:
  ...
  virtual void visit(const VERTEX_ID v_id, const bool mark = true);
  virtual bool isVisited(const VERTEX_ID v_id) const;
  virtual string print(const VERTEX_ID start, const VERTEX_ID end, const LinkList & backtrack) const;
...
class GraphTravellerDfsPath : public GraphTravellerDfs {
 ...
 private:
  ...
  size_t uncoveredBranches(const Graph & g, const VERTEX_ID v, const size_t steps) const;
  // get the new edge which has most possibility to walk on new path
  // by calculating the uncovered branches of its children
  LINK_ID mostChoice(const Graph & g, const VERTEX_ID v, const size_t steps, LINK_ID & e) const;
  friend class IGraphTraveller;
};
...
class GraphTravellerEuler : public IGraphTraveller {
   ...
   * another vertex w there must be an unused edge leaving w. The tour formed in this
   * way is a closed tour, but may not cover all the vertices and edges of the initial graph.
   *
 ...
 protected:
  GraphTravellerEuler() :
    m_start(0), m_random(true), m_visit_table(NULL) {
  };
