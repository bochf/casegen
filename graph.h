...
class Graph {
 ...
 private:
  ...
  VertexList m_forks; // the vertices set which all the vertices' in degree less than out degress
  VertexList m_arrows; // the vertices set which all the vertices' in degree greater than out degress
  VertexList m_equals; // the vertices set which all the vertices' in degree equals to out degress

  void divide(); // divide the vertices into 2 parts, fork vertices and arrow vertices
  ...
  // a and be might be directed connected or intermedia connected through multipl
  // balanced vertices
  // this function is to increase out degree of a and in degree of b
  // by repeating cloning the path in limited times
  // either a or b will become balanced vertex (or both get to balance at same time)
  // because we will add both in edge and out edge at same time on the intermedia
  // vertex if a and b are not directly connected, so the operation will not break
  // the balance status of the intermedia vertices
  ...
  inline static bool compareByLength(const LinkList * path1, const LinkList * path2) {
